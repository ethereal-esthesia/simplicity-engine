...
usage() {
  cat <<'EOF'
Usage: tools/parallels/setup.sh [options]
...
guest_repo_exists() {
  ...
  if [[ "$TARGET" == "windows" ]]; then
    ...
    prlctl exec "$vm_name" --current-user test -f "${guest_repo}/tools/parallels/setup.sh" >/dev/null 2>&1 </dev/null
  fi
}
...
{
  printf '# Generated by tools/parallels/setup.sh. This file is gitignored.\n'
  printf 'VM_NAME=%q\n' "$vm_name"
  printf 'GUEST_REPO=%q\n' "$guest_repo"
  printf 'HOST_REPO=%q\n' "$host_repo"
  printf 'PRESET=%q\n' "$preset"
