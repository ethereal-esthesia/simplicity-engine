...
usage() {
  cat <<'EOF'
Usage: ./scripts/run_target.sh <target> [options]
   or: ./scripts/run_target.sh --target <target> [options]
...
case "$TARGET" in
  host|macos)
    exec "${SCRIPT_DIR}/run.sh" ${PASS_THROUGH[@]+"${PASS_THROUGH[@]}"}
    ;;
  ios-phone|ios-iphone)
    exec "${SCRIPT_DIR}/run_ios_iphone.sh" ${PASS_THROUGH[@]+"${PASS_THROUGH[@]}"}
    ;;
  *)
